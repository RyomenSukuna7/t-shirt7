...
export default function PersistentDrawerLeft({children}) {
  ...
  React.useEffect(() => {
    ...
    let countd=atob(count);

    

    if (countd === null || countd === "1") {
      ...
      localStorage.setItem("count", btoa("2"));
    }
  }, []);
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <AppBar position="fixed" open={open} style={{borderRadius:"20px",backgroundColor:"black"}}>
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div" id='complete'>
          Thread Symphony
          </Typography>
        </Toolbar>
      </AppBar>
      ...
      >
        ...
        <Button startIcon={<GamepadIcon />} onClick={()=>router.push("/GamePadDataSend")}>Game Pad</Button>
        <Divider  color="white"/>
        <br/><br/>
        <Divider color="white"/>
